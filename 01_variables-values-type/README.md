...
Using formatting allow us to write idiomatic Go code, which we all can use.

## Bare bones of a Go Program
...
Go also prevents/asks us to not use polluted code. We can use "_" to throwaway the code instead of declaring an unused variable.
...
In CompSci, a composite data type allows us to compose/aggregate other values of a certain type.
...
Understanding initialization and the zero value in Golang. Some review about declaring/assigning.
...
func main() {
    q = 53 //Assigning or initialize a value to that variable
    fmt.Println(q)
}
...
Things in a computer are stored in memory. In a computer we have memory (like a post office with P.O. boxes). We have all of these values stored in different P.O. boxes.
...
Looking more into how we read the standard library documentation with the fmt package.
